...
class AuditService:
    ...
    async def get_audit_summary(self, user_mail: str) -> Dict[str, Any]:
        """獲取用戶稽核日誌摘要"""
        return await self.audit_repository.get_user_log_summary(user_mail)
    ...
    async def get_system_audit_overview(self) -> Dict[str, Any]:
        ...
        for user_mail in all_users:
            user_summary = await self.get_audit_summary(user_mail)
